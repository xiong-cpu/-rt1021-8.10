...
void iic_iomuxc(IICN_enum iic_n, IIC_PIN_enum sda_pin, IIC_PIN_enum scl_pin)
{
    switch(iic_n)
    {
        ...
        case IIC_4:
        {
            if      (IIC4_SDA_C10 == sda_pin) iomuxc_pinconf(B9  ,ALT2,IIC_PIN_CONF);
            else if (IIC4_SDA_D23 == sda_pin) iomuxc_pinconf(D14 ,ALT3,IIC_PIN_CONF);
            else    assert(0);                                  
                                                                
            if      (IIC4_SCL_C11 == scl_pin) iomuxc_pinconf(B8  ,ALT2,IIC_PIN_CONF);
            else if (IIC4_SCL_D22 == scl_pin) iomuxc_pinconf(D13 ,ALT3,IIC_PIN_CONF);
            else    assert(0);
        }break;
